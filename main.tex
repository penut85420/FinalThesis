...
\section*{摘要}
\paragraph{}
中文文本蘊涵關係辨識是自然語言處理的核心技術之一。目前已出現許多英文和中文文本蘊涵關係辨識的實驗資料集，利用現代深度學習的技術可在這些資料集上獲得優秀的辨識效能。但是\ NTCIR\ RITE\ 中文資料集乃針對特殊語言現象所設計，因此尚未有夠好的辨識成效。

\paragraph{}
本論文主要目的是研究如何提昇\ RITE\ 中文資料集的深度學習辨識效能。我們擴展了\ RITE\ 的官方資料，也使用數個已知的大型文本蘊涵實驗資料集當作額外的訓練資料。我們採用兩階段預訓練的方法來運用這些資料集。

\paragraph{}
此外，本論文也提出了兩種產生虛擬訓練資料的方法。我們設計了一組生成各種蘊涵關係文句配對的規則，從大型語料庫中產生大量的虛擬訓練資料。我們也提出一個方法用\ GPT-2\ 來產生同義或反義的文句配對。

\paragraph{}
實驗結果顯示，使用越大型的訓練資料，對於辨識效能的改進越多。利用\ MNLI\ 和\ CNLI\ 這類大型訓練資料集來預訓練系統、再用擴增版\ RITE\ 訓練資料來微調所得的系統會達到最佳效能，其\ F1-score\ 在\ RITE2\ 和\ RITE-VAL\ 測試集分別為\ 0.7154\ 和\ 0.5797。

\paragraph{}
在訓練資料資料數量不夠多的時候，加入虛擬訓練資料是有幫助的。實驗證明虛擬訓練資料改善了訓練自\ RITE\ 訓練資料或\ CNLI\ 的系統，但是無法改善訓練自\ MNLI\ 這種大型資料集的系統。

\paragraph{關鍵詞：文本蘊涵關係辨識、自然語言推論、兩階段預訓練、深度學習}
...
\section*{Abstract}
\paragraph{}
Recognizing textual entailment is a core technology in natural language processing. Many RTE datasets have been constructed in English and Chinese. Modern deep learning approach achieves great performance on these datasets, but not for NTCIR RITE datasets which are designed for capturing specific linguistic phenomena.

\paragraph{}
This thesis focuses on improving deep learning systems evaluated on the RITE datasets. We prepare the expanded version of the RITE datasets and use other large-scaled RTE datasets as additional training data. We introduce the two-step pre-training technique to utilize these datasets.

\paragraph{}
Moreover, we propose two approaches to produce pseudo training data. A set of RTE-pair constructing rules are designed to create new data from a large corpus. We also propose a method to use GPT-2 to generate synonymous or antonymous sentence pairs.

\paragraph{}
The experimental results show that using larger training sets will lead to better performance. Pre-training with MNLI and CNLI, two  large RTE datasets, and fine-tuning with the expanded RITE datasets achieves the best performance with F1-scores of 0.7154 and 0.5797 for NTCIR RITE2 and RITE-VAL, respectively. They outperform the state-of-the-art systems evaluating on RITE2 and RITE-VAL datasets.

\paragraph{}
Adding pseudo training data can improve the performance of a RTE system trained with a smaller training set, such as the original RITE training set or CNLI. But it is not helpful to a RTE system trained with a larger training set, such as MNLI.

% \paragraph{}
% Our experiment result shows that BERT pre-trained with cross-lingual large-scaled datasets and fine-tuned with the expanded RITE datasets have best performance. The system achieves F1-scores of 0.7154 and 0.5797 to RITE2 and RITE-VAL, and greatly outperforms the best system in NTCIR RITE2 and RITE-VAL Traditional Chinese subtasks.
...
% ?? 上傳的時候 摘要不要放 keyword
\paragraph{Keywords} recognizing textual entailment, natural language inference, two-step pre-training, deep learning.
...
The definition of recognizing textual entailment (RTE), or natural language inference (NLI), is the entailment-related relation between two sentences. That is, given that the state described in the first sentence is true, the state described in the second sentence will be true or not.
...
\begin{table}[H]
  ...
  \begin{tabular}{|L{2cm}|L{13cm}|}
    \hline
    \multirow{2}{*}{Premise} & The way we try to approach it is to identify every legal problem that a client \\
    & has. \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabular}{|L{2cm}|L{13cm}|}
    \hline
    \multirow{2}{*}{Premise} & There was food for all, and houses had been conjured hastily to shelter the \\
    & people. \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabular}{|L{2cm}|L{13cm}|}
  \hline
  \multirow{3}{*}{Premise} & 約瑟夫·傅立葉是十九世紀法國數學家及物理學家。 \\
   & (Joseph Fourier was a French mathematician and physicist in the 19th \\
   & century.) \\ \hline
  \multirow{3}{*}{Hypothesis} & 十九世紀法國的約瑟夫·傅立葉是數學家、物理學家。 \\
   & (Joseph Fourier born in French in the 19th century was a mathematician and \\
   & physicist.) \\ \hline
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{|L{2cm}|L{13cm}|}
  \hline
  \multirow{3}{*}{Premise} & 約瑟夫·傅立葉是十九世紀法國數學家、物理學家。 \\
   & (Joseph Fourier was a French mathematician and physicist in the 19th \\
   & century.) \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabular}{|L{2cm}|L{13cm}|}
  \hline
  \multirow{3}{*}{Premise} & 喬治·盧卡斯最著名的作品是《星際大戰》和《法櫃奇兵》系列。 \\
   & (The most famous works of George Lucas are Star Wars and Indiana Jones \\
   & series.) \\ \hline
  \multirow{3}{*}{Hypothesis} & 喬治·盧卡斯最著名的作品不是《星際大戰》和《法櫃奇兵》系列。 \\
...
\paragraph{}
In the recognizing textual entailment task, we concatenated the sequence of word vectors of the premise and the hypothesis, a special token ``\texttt{[SEP]}'' will be put in the middle as the separator.

\paragraph{}
There are many methods to represent the word embedding, we will introduce them in the following sections.
\label{section:word_embedding}
...
\begin{table}[H]
  ...
  \multirow{3}{*}{MNLI $\Rightarrow$ CNLI} & RITE-Original & 0.6952 & 0.6955 & 0.5811 & 0.5650 \\ \cline{2-6}
...
%   \multicolumn{1}{|c|}{\multirow{2}{*}{System}} & \multicolumn{2}{c|}{RITE2} & \multicolumn{2}{c|}{RITE-VAL} \\ \cline{2-5}
%   \multicolumn{1}{|c|}{} & Training & Test & Training & Test \\ \hline
%   GRU & 0.3731 & 0.2587 & 0.4227 & 0.2351 \\ \hline
%   SWS v1 & 0.5472 & 0.3981 & 0.4133 & 0.3327 \\ \hline
%   SWS v2 & 0.4975 & 0.3699 & 0.4224 & 0.3113 \\ \hline
%   SWS v2 CNLI & 0.4680 & 0.4643 & 0.4446 & 0.3995 \\ \hline
%   \end{tabular}
% \end{table}
...
\paragraph{}
In Sec. \ref{section:pseudo_dataset}, we propose several pseudo training data. We use these datasets as the pre-training dataset. RPNLI denotes Rule-Based Pseudo-NLI. GPNLI denotes GPT-2 Pseudo-NLI. APNLI is the combination of RPNLI and GPNLI. Table \ref{result:pseudo_nli_bert} shows the performance of BERT pre-trained with these pseudo-NLI datasets.

\paragraph{}
Systems in the first group G\#1 in Table \ref{result:pseudo_nli_bert} are pre-trained with one of the pseudo-NLI datasets and fine-tuned with the original RITE training sets. Similarly, only the fine-tuning step is in the fashion of cross-validation. As we can see in Table \ref{result:pseudo_nli_bert}, using any pseudo-NLI dataset can improve the system without pre-training. Using RPNLI or APNLI achieves the most improvement in different cases.

\paragraph{}
Systems in the second group G\#2 in Table \ref{result:pseudo_nli_bert} are pre-trained with the combination of CNLI-CT and one of the pseudo-NLI datasets, and then fine-tuned with the original RITE training sets. As we can see, adding any pseudo-NLI dataset can improve the system pre-trained with CNLI-CT only. Using RPNLI or APNLI achieves the most improvement in different cases.

\paragraph{}
Systems in the third group G\#3 in Table \ref{result:pseudo_nli_bert} are built in two-step pre-training. We first pre-train the model with one of the pseudo-NLI datasets, and then pre-train with CNLI-CT, and finally fine-tuned with the original RITE training sets. As we can see, systems using any pseudo-NLI dataset as the first pre-training dataset do not outperform the systems trained with the combination of NLI datasets in G\#2.

\paragraph{}
Systems in the fourth group G\#4 in Table \ref{result:pseudo_nli_bert} are pre-trained with the combination of MNLI and one of the pseudo-NLI datasets, and then fine-tuned with the original RITE training sets. As we can see, adding any pseudo-NLI dataset can not improve the system pre-trained with MNLI only. We conclude that the pseudo training data is not useful when the size of pre-training dataset is large enough.

% In most cases, the performance is better when using RPNLI in the pre-training dataset. When mixing with CNLI-CT, the performance of the system is improved. When mixing with MNLI, the performance of the system is decreased. We conclude that the pseudo training data is not useful when the size of pre-training dataset is large enough.

% ?? 加一個 APNLI => CNLI-CT ，與其他 two-step 合在一起變成第三組
\begin{table}[H]
  \centering
  \setlength{\extrarowheight}{-3pt}
  \caption{Performance of BERT Pre-Trained with Pseudo-NLI Datasets}
  \label{result:pseudo_nli_bert}
  \begin{tabular}{|c|l|C{1.5cm}|C{1.5cm}|C{1.5cm}|C{1.5cm}|}
  \hline
  \multirow{2}{*}{G\#} & \multicolumn{1}{c|}{\multirow{2}{*}{Pre-Training Dataset}} & \multicolumn{2}{c|}{RITE2} & \multicolumn{2}{c|}{RITE-VAL} \\ \cline{3-6}
   & \multicolumn{1}{c|}{} & Training & Test & Training & Test \\ \hline
  \multirow{4}{*}{1} & RITE-Original & \textit{0.5582} & \textit{0.4403} & \textit{0.4907} & \textit{0.3767} \\ \cline{2-6}
   & RPNLI & 0.6541 & \textbf{0.5777} & \textbf{0.5524} & 0.4257 \\ \cline{2-6}
   & GPNLI & 0.6283 & 0.5378 & 0.5156 & 0.4210 \\ \cline{2-6}
   & APNLI & \textbf{0.6695} & 0.5659 & 0.5258 & \textbf{0.4440} \\ \hline
   \hline
  \multirow{4}{*}{2} & CNLI-CT & \textit{0.6848} & \textit{0.6426} & \textit{0.5915} & \textit{0.5146} \\ \cline{2-6}
   & CNLI-CT + RPNLI & 0.6908 & \textbf{0.6710} & 0.5992 & \textbf{0.5423} \\ \cline{2-6}
   & CNLI-CT + GPNLI & 0.6687 & 0.6409 & 0.5964 & 0.5364 \\ \cline{2-6}
   & CNLI-CT + APNLI & \textbf{0.6940} & 0.6595 & \textbf{0.6114} & 0.5335 \\ \hline
   \hline
   \multirow{3}{*}{3} & RPNLI $\Rightarrow$ CNLI-CT & 0.6835 & 0.6497 & 0.5746 & 0.5253 \\ \cline{2-6}
   & GPNLI $\Rightarrow$ CNLI-CT & 0.6797 & 0.6499 & \textbf{0.5946} & 0.5328 \\ \cline{2-6}
   & APNLI $\Rightarrow$ CNLI-CT & \textbf{0.6866} & \textbf{0.6616} & 0.5732 & \textbf{0.5414} \\ \hline
   \hline
  \multirow{4}{*}{4} & MNLI & \textit{\textbf{0.6919}} & \textit{\textbf{0.7107}} & \textit{\textbf{0.5713}} & \textit{\textbf{0.5561}} \\ \cline{2-6}
   & MNLI + RPNLI & 0.6796 & 0.6841 & 0.5576 & 0.5222 \\ \cline{2-6}
   & MNLI + GPNLI & 0.6818 & 0.6619 & 0.5603 & 0.4947 \\ \cline{2-6}
...
\paragraph{}
This thesis focuses on improving deep learning on the NTCIR RITE Chinese datasets. The proposed approaches include pre-training with large-scaled NLI datasets and creating pseudo training datasets.

\paragraph{}
The steps to train classifiers by deep learning in this thesis are as follows. First, large-scaled NLI datasets such as MNLI or CNLI are used to pre-train models. One additional pre-training step can be performed by using another large-scaled NLI dataset. The expanded version of the RITE training sets are used to fine-tune the models.

\paragraph{}
Two approaches to produce pseudo training data are proposed. A set of RTE-pair constructing rules are designed to create new data from a large corpus. We also propose a method to use GPT-2 to generate synonymous or antonymous sentence pairs.
...
\paragraph{}
In the future, we would like to study how to generate pseudo RTE sentence pairs that can capture linguistic phenomena introduced in the NTCIR RITE-VAL task. Hopefully, these data can improve the RTE systems on the RITE-VAL test set.

\paragraph{}
We are also interested in applying the same technique proposed in this thesis to other languages covered by the multilingual version of BERT.
...
\paragraph{}
Table \ref{result:bert-rite2-dev} shows the individual F1-scores of BFCI classes on the RITE2 training set. Table \ref{result:bert-rite2-test} shows the individual F1-scores of BFCI classes on the RITE2 test set. Table \ref{result:bert-rite-val-dev} shows the individual F1-scores of BFCI classes on the RITE-VAL training set. Table \ref{result:bert-rite-val-test} shows the individual F1-scores of BFCI classes on the RITE-VAL test set.
...
\begin{table}[H]
  ...
  CNLI-CT (w/o fine-tuning) & 0.5443 & 0.7003 & 0.4158 & 0.4902 & 0.5377 \\ \hline
...
\begin{table}[H]
  ...
  CNLI-CT (w/o fine-tuning) & 0.6797 & 0.6667 & 0.5035 & 0.5303 & 0.5950 \\ \hline
...
\begin{table}[H]
  ...
  \hline
  \multicolumn{1}{|c|}{Pre-Training Dataset} & \multicolumn{1}{c|}{B-F1} & \multicolumn{1}{c|}{F-F1} & \multicolumn{1}{c|}{C-F1} & \multicolumn{1}{c|}{I-F1} & \multicolumn{1}{c|}{Micro F1} \\ \hline
  CNLI-CT (w/o fine-tuning) & 0.7024 & 0.5022 & 0.4863 & 0.2157 & 0.4766 \\ \hline
...
\begin{table}[H]
  ...
  \hline
  \multicolumn{1}{|c|}{Pre-Training Dataset} & \multicolumn{1}{c|}{B-F1} & \multicolumn{1}{c|}{F-F1} & \multicolumn{1}{c|}{C-F1} & \multicolumn{1}{c|}{I-F1} & \multicolumn{1}{c|}{Macro F1} \\ \hline
  CNLI-CT (w/o fine-tuning) & 0.6110 & 0.5506 & 0.5219 & 0.3135 & 0.4993 \\ \hline
