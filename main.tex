...
\begin{figure*}
  ...
  \end{minipage}
  % (a) An Example from the RITE2 Training Set
  % \end{subfigure}
  % ?? subfigure 圖標題在下 調整圖標題間距
  % ?? 解決跳號問題
  % \begin{subfigure}{\linewidth}
    \begin{minipage}{\linewidth}
    \begin{lstlisting}[language=XML]
    ...
    \end{minipage}
  % (b) An Example from the RITE-VAL Training Set
  % \end{subfigure}
\end{figure*}
...
\begin{figure}[H]
  ...
  \caption[Hierarchical Structure of WordNet]{Hierarchical Structure from WordNet}
  \label{fig:wordnet}
\end{figure}
...
% ?? 表 12 多一行 MLP ALL
% ?? 表 13 跟 表 14 會消失
% ?? 表 12 Training Data 改成 Original
% 主軸： 1. nn classifier input by features (MLP)
% 2. compare with SVM
% ?? 補上 MNLI => CNLI => Inter-Dataset 實驗
\begin{table}[H]
  \centering
  \setlength{\extrarowheight}{-3pt}
...
\begin{figure}[H]
  ...
  \caption[Cosine Similarity Threshold Comparison]{Comparison of the Cosine Similarity Threshold of the Similarity-Based Word Sequence}
  \label{fig:threshold}
\end{figure}
...
% ?? 寫清楚 BERT Base & BERT Large 的效能不同
\paragraph{}
...
We try to combine the BERT model with the similarity-based word sequence and the machine learning features to see how these features will impact the performance. We use the MNLI dataset and the CNLI-TW dataset as the benchmark.
...
\begin{table}[H]
  ...
  \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{Dev} & \multicolumn{1}{c|}{Test} & \multicolumn{1}{c|}{Dev} & \multicolumn{1}{c|}{Test} \\ \hline
  MNLI & 0.5713 & 0.5561 & 0.6919 & 0.7107 \\ \hline
  MNLI \& MLFT & 0.5216 & 0.5301 & 0.6697 & 0.7032 \\ \hline
  MNLI \& SWS & 0.3275 & 0.2614 & 0.4459 & 0.3473 \\ \hline
  MNLI \& MLFT \& SWS & 0.4812 & 0.4414 & 0.5739 & 0.6077 \\ \hline \hline
  MNLI $\Rightarrow$ CNLI & 0.5811 & 0.5650 & 0.6952 & 0.6955 \\ \hline
  MNLI $\Rightarrow$ CNLI \& MLFT & 0.5375 & 0.5440 & 0.6444 & 0.6929 \\ \hline
  MNLI $\Rightarrow$ CNLI \& SWS & 0.3809 & 0.3204 & 0.4581 & 0.3911 \\ \hline
  MNLI $\Rightarrow$ CNLI \& MLFT \& SWS & 0.5071 & 0.4908 & 0.6004 & 0.6342 \\ \hline
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption[Pseudo Training Data Size Curve]{Relationship Between Pseudo Training Data Size and the Performance of the Recurrent Neural Network}
